...
import {
  ...
  generateRAPIDReviewSummary,
  generateLegacyReviewSummary,
  calculateOverallCompletion,
  formatAnswer
} from '../common/ResponseReviewModalUtils'
...
const ResponseReviewModal: React.FC<ResponseReviewModalProps> = ({
  ...
  if (questions && !rapidQuestions) {
    ...
    const legacySummary = useMemo((): CategoryReviewSummary[] => {
      return generateLegacyReviewSummary(questions, responses);
    }, [questions, responses]);
  ...
  const categoryReviewSummary = useMemo((): CategoryReviewSummary[] => {
    return generateRAPIDReviewSummary(rapidQuestions, responses);
  }, [rapidQuestions, responses])

  // Calculate overall completion status
  const overallCompletion = useMemo(() => {
    return calculateOverallCompletion(categoryReviewSummary);
  }, [categoryReviewSummary])
